...
export default async function (t : Test) {
  t.test('Stress test of signature validation.', async t => {
    ...
    for (let i = 0; i < rounds; i++) {
      ...
      const utils_valid_1 = ecc.verify(tiny_sig, message, tiny_pub,   { throws : false })
      const utils_valid_2 = ecc.verify(noble_sig, message, noble_pub, { throws : false })
      const utils_valid_3 = ecc.verify(utils_sig, message, utils_pub, { throws : false })
