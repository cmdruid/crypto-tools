...
const msg = Buff.raw(ec.encode('test'), 32)

const sig1A = ecc.sign(msg, k1.raw)
...
const T3 = ecc.pointAddScalar(K1.raw, tweak, true)

if (t2 === null || T2 === null || T3 === null) {
  throw TypeError('Encountered null value!')
}

const sig2A = ecc.sign(msg, t1.raw)
const sig2B = ecc.sign(msg, t2)

export default function ECCTest(t) {
  t.test('Testing ECC Primitives', t => {
    t.plan(13)
    t.deepEqual(k1.raw, seed.raw, 'should be equal seed values')
    t.deepEqual(K1.raw, K2, 'should be equal points')

    t.equal(ecc.verify(msg, K2, sig1A), true, 'sig1A and K2 should be valid')
    t.equal(ecc.verify(msg, K1.raw, sig1B), true, 'sig1B and K1 should be valid')

    t.deepEqual(t1.raw, new Uint8Array(t2), 'our tweaked scalar should match ecc scalar')

    t.deepEqual(T1.raw, T2, 'our tweaked point should match ecc point')
    t.deepEqual(T2, T3, 'ecc points should match when tweaking point or scalar')

    t.equal(ecc.verify(msg, T1.raw, sig2A), true, 'T1 should verify signature 2A')
    ...
    t.equal(ecc.verify(msg, T1.raw, sig2B), true, 'T1 should verify signature 2B')
